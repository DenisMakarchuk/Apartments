...
export class OrderUserService {
    ...
    formationOrder(order: AddOrder): Observable<OrderView> {
    ...
    protected processFormationOrder(response: HttpResponseBase): Observable<OrderView> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
...
export class ApartmentImageService {
    ...
    uploadImage(image: FileParameter | null | undefined, apartmentId: string | null | undefined): Observable<string> {
